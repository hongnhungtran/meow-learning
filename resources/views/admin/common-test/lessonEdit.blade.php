...
<div class="row">
    <div class="col-md-12">
        <div class="box box-info">
            ...
            <form class="form-horizontal" method="post" action="{{action('Admin\CommonTestController@updateLesson', $lesson[0]->lesson_id)}}">
                {{csrf_field()}} 
                <div class="box-body">
                            ...
                            <select class="form-control" name="level">
								@if ($levels->count())
    								@foreach($levels as $level)
    								    <option value="{{ $level->level_id }}" {{ $level->level_id == $lesson[0]->level_id ? 'selected="selected"' : '' }} > {{ $level->level_name }} </option> 
    								@endforeach   
								@endif
							</select>
                        </div>
